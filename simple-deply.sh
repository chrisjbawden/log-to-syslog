...
echo ""
docker exec -it "$docker_name" bin/logstash-plugin install logstash-output-syslog

echo ""
# Copy the logstash.conf into the container
docker cp logstash.conf "$docker_name":/usr/share/logstash/pipeline/logstash.conf

sleep 1
echo ""
# Restart container silently
echo restarting container ...
docker restart "$docker_name" > /dev/null 2>&1

echo ""
# Cleanup the local logstash.conf file
rm logstash.conf
